...
void ControlState_Initialize(ControlState *control_state) {
	control_state->current_mode = INSERT;
    ...
    insert_mode->creating_shape = false;
...
bool ControlState_ToggleMode(ControlState *control_state, RenderingState *rendering_state) {
	switch (control_state->current_mode) {
		case INSERT:
            if (control_state->insert_mode.creating_shape)
                return false;

			control_state->current_mode = EDIT;
        	break;
        case EDIT:
			control_state->current_mode = INSERT;
            break;
...
void ControlState_Update_Mode(ControlState *control_state, RenderingState *rendering_state, GLFWwindow *window) {
	switch (control_state->current_mode) {
        case INSERT:
        	InsertMode_Update(control_state, rendering_state, window);
            break;
        case EDIT:
        	EditMode_Update(control_state, rendering_state, window);
            break;	
	}
    ...
    if (control_state->last_keyboard.space_button_state == GLFW_PRESS && control_state->current_keyboard.space_button_state == GLFW_RELEASE)
        ControlState_ToggleMode(control_state, rendering_state);
}
